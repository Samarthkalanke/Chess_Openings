...
<body>
    ...
    <script>
        ...
        // Array of KingsIndian positions for the chess pieces
        var KingsIndianPositions = [
        ...
        // Initialize the chess board
        function KingsIndianChessBoard() {
            var chessHTML = `<table>`;
            for (var row = 1; row <= 8; row++) {
                chessHTML += `<tr>`;
                for (var col = 1; col <= 8; col++) {
                    var squareClass = (row + col) % 2 === 0 ? "white-square" : "black-square";
                    //if (row === 1) squareClass = "black-square"; // Make the bottom side black
                    var piece = getKingsIndianPieceIcon(row, col);
                    chessHTML += `<td><div class="chess-square ${squareClass}" id="r${row}c${col}">${piece}</div></td>`;
                }
                chessHTML += `</tr>`;
            }
        ...
        function getKingsIndianPieceIcon(row, col) {
            for (var i = 0; i < KingsIndianPositions.length; i++) {
                var position = KingsIndianPositions[i];
...
> # Youtube Vids(GothamChess and Others)
    > Gotham Chess 10 min overview https://www.youtube.com/watch?v=kK0cq6UBt1Y
    > Gotham Chess Beth Harmon stuff https://www.youtube.com/watch?v=iGCSfVDYhh0
    > Simplify Chess openings https://www.ragchess.com/how-to-play-the-kings-indian-defense/